...
\textcolor{blue}{Yes, we have created an additional appendix table (S3) that summarizes bias in $b$ (the preferential sampling parameter), as well as bias in the species-habitat parameters.  We don't look at bias in the sampling probability parameters because data were simulated by sampling without replacement (with a fixed number of sampled survey units), whereas the fitted model was Bernoulli (which assumes the number of sampled units is random).  Thus parameters would not be directly comparable.  We also provide convergence statistics in this appendix. %We note that the bias in the slope parameter of the species-habitat relationship %is likely because of the well-known confounding that occurs between regression %coefficients and spatially autocorrelated random effects in spatial models %\citep[e.g.][]{ReichEtAl2006,HodgesReich2010}.
}
...
\textcolor{blue}{We now include these known zero values in all models.}
...
\textcolor{blue}{We now present models in order of $\Delta$AIC.}

Fig. 3: In my version, the blue and green dashed lines do not show up in the legend.

\textcolor{blue}{Strange, this is not the case in our version.  Guessing it is an issue with PDF compilation on the MEE submission website.  We'll work with copy editors to ensure it looks right as it heads into production.}
