...
load("c:/users/paul.conn/git/bass/AlaskaBeringData2012_17April2014_shelf_fixed.Rda")  #boss grid, ice data
...
Flt.table=data.frame(id=Flt.ids,min.time=rep(0,length(Flt.ids)),day=rep(0,length(Flt.ids)))
for(irow in 1:nrow(Flt.table)){
  Flt.table[irow,"min.time"]=min(Points[which(Points$flightid==Flt.ids[irow]),]$dt_utc)
  Flt.table[irow,"day"]=min(yday(Points[which(Points$flightid==Flt.ids[irow]),]$dt_utc))
}
#note day = 7 corresponds to 10 April.
plot(Data$Grid[[1]])
#plot(Tracks[which(Tracks$flightid=="12_OtterFl01"),],add=TRUE,col="blue")

#limit to 2012 and April 10-16 (note ice is more constant this time of year 0.67 --> 0.62)
#note 10 Apr corresponds to yday=101
myfun<-function(x)strsplit(x,'_')[[1]][1]
Year=sapply(as.character(Flt.table[,"id"]),myfun)
Yr12.ind=Day.ind=rep(0,nrow(Flt.table))
Yr12.ind[which(Year=="12")]=1
Day.ind[which(Flt.table$day %in% c(101:107))]=1

Flights=Flt.ids[Yr12.ind & Day.ind]
Hotspots=Hotspots[which(Hotspots[["flightid"]] %in% Flights),]
...
save(Tracks,file="2012_Tracks_for_bearded_prefsamp_analysis.Rdata")
...
#Hotspots=Hotspots[-which(int==FALSE),]  #all intersect, so skip this step for this dataset
...
#Combine area from cells surveyed by more than one flight segment 
Mapping=rep(0,nrow(Length.df))  
DayHour=matrix(0,nrow(Length.df),2) #mean day, hour
Area.trans=rep(0,nrow(Length.df))
icounter=0
icounter2=0
I.processed=rep(0,nrow(Length.df))
Hour.trans=Length.df[,"Hour"]
Hour.trans[which(Hour.trans<12)]=Hour.trans[which(Hour.trans<12)]+24
while(sum(I.processed)<nrow(Length.df)){
  ...
  if(I.processed[icounter]==0){
    ...
    cur.hour=Hour.trans[icounter]
    Area.trans[icounter2]=cur.area
    Mapping[icounter2]=cur.poly
    DayHour[icounter2,]=c(cur.day,cur.hour)
    Which.eq=which(Length.df[,"poly"]==cur.poly)
    if(length(Which.eq)>1){
      Area.trans[icounter2]=Area.trans[icounter2]+sum(Length.df[Which.eq[2:length(Which.eq)],"area"])
      DayHour[icounter2,1]=round(mean(c(DayHour[icounter2,1],Length.df[Which.eq[2:length(Which.eq)],"Day"])))
      DayHour[icounter2,2]=round(mean(c(DayHour[icounter2,2],Hour.trans[Which.eq[2:length(Which.eq)]])))
...
DayHour=DayHour[1:icounter2,]
DayHour[which(DayHour[,2]>23),2]=DayHour[which(DayHour[,2]>23),2]-24

Count.data=data.frame(matrix(0,length(Mapping),3))
colnames(Count.data)=c("Cell","AreaSurveyed","Count")
Count.data[,1]=Mapping
Count.data[,2]=Area.trans
for(irow in 1:nrow(Count.data)){
  Which.entries=which(Cell.id.hs==Mapping[irow])
...
Effort=list(Mapping=Mapping,Area.trans=Area.trans,Area.hab=Area.hab,Count.data=Count.data,DayHour=DayHour)
save(Effort,file="Effort2012_bearded_PrefSamp_21Dec2015.Rdata")
